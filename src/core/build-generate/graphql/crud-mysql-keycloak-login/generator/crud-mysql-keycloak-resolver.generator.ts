...
export const GenerateCrudResolverKeycloakLoginModule = async (entityName: string, pathEntity: string) => {

  const filePath = path.resolve(pathEntity, `${singular(entityName)}.resolver.ts`)

  const rendered = CrudResolverGrapHQLKeyCloakLoginTemplate(entityName)

  await writeFile(rendered, filePath)
}

export const CrudResolverGrapHQLKeyCloakLoginTemplate = (entityName: string) => {
