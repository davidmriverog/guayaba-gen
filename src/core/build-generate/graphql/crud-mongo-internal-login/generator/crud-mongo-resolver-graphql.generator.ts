...
export const GenerateCrudResolverInternalLoginMongoModule = async (entityName: string, pathEntity: string) => {

  const filePath = path.resolve(pathEntity, `${singular(entityName)}.resolver.ts`)

  const rendered = CrudResolverGrapHQLInternalLoginMongoTemplate(entityName)

  await writeFile(rendered, filePath)
}

export const CrudResolverGrapHQLInternalLoginMongoTemplate = (entityName: string) => {
