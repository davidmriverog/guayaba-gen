...
export class GQLCrudModule extends Command {
  ...
  static description = 'Generador de Module GRAPHQL - CRUD (INTERNAL LOGIN)'
  ...
  async run() {
    ...
    const { args, flags } = this.parse(GQLCrudModule)
    ...
    await GenerateCrudResolverGQLGuardModule(name, crudModelPath)
